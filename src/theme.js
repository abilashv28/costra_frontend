export const theme = {
  layout: {
    ...
    // Bottom Nav (Mobile)
    bottomNav: "fixed bottom-0 left-0 right-0 z-40 bg-white/90 backdrop-blur-lg border-t border-gray-200 flex justify-start sm:justify-around items-center p-2 pb-[max(0.5rem,env(safe-area-inset-bottom))] shadow-[0_-4px_15px_-3px_rgba(0,0,0,0.1)] md:hidden transition-transform duration-300 overflow-x-auto overflow-y-hidden snap-x",
    ...
    // Bottom nav links
    bottomNavLink: "flex flex-col items-center gap-1 p-2 min-w-[72px] rounded-xl text-slate-500 hover:bg-slate-100 hover:text-slate-900 transition-colors snap-center flex-shrink-0",
    bottomNavLinkActive: "flex flex-col items-center gap-1 p-2 min-w-[72px] rounded-xl text-blue-600 bg-blue-50 font-medium snap-center flex-shrink-0",
  },
  button: {
